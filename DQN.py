...
output = tf.layers.dense(dense, NUM_ACTIONS, activation=tf.nn.softmax)
...
with tf.Session() as sess:
	...
	for i in range(num_iter):
		...
		if(rand>epsilon):
			...
			_output = sess.run(output, feed_dict = {state_inp:final_state})
		...
		target = reward_buff + (gamma * np.argmax(_output_state_buff) * (1 - done))
